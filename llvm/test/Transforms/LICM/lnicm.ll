...
; RUN: opt -aa-pipeline=basic-aa -passes='loop-mssa(loop-interchange)' -cache-line-size=64 \
; RUN:     -verify-memoryssa -S %s | FileCheck %s --check-prefixes INTC
; RUN: opt -aa-pipeline=basic-aa -passes='loop-mssa(lnicm,loop-interchange)' -cache-line-size=64 \
; RUN:     -verify-memoryssa -S %s | FileCheck %s --check-prefixes LNICM
; RUN: opt -aa-pipeline=basic-aa -passes='loop-mssa(licm,loop-interchange)' -cache-line-size=64 \
; RUN:     -verify-memoryssa -S %s | FileCheck %s --check-prefixes LICM
