...
; RUN:     -verify-memoryssa -verify-loop-lcssa -S | FileCheck %s
; RUN: opt < %s -passes=loop-interchange -cache-line-size=64 -da-disable-delinearization-checks \
; RUN:     -verify-memoryssa -pass-remarks-missed='loop-interchange' -verify-loop-lcssa -S \
; RUN:     | FileCheck -check-prefix=CHECK-DELIN %s
